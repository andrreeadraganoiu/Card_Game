...
void razboiMove(SList* Player, int min)
{
    ...
    for(int i = 1; i <= min; i++)
        (*Player)->razboi_card = (*Player)->razboi_card->next; //Player->razboi_card = valoarea cartii care decide cine a castigat acest razboi
    (*Player)->intermediate = (*Player)->razboi_card->next;    //Player->intermediate = valoarea dinaintea valorii care decide cine a castigat razboiul
